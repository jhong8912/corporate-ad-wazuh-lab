...
* **Milestones (July 23):** Engineered custom Wazuh detection logic (`local_rules.xml`, Rule ID `100001`) targeting Sysmon Event ID 1 telemetry streams. Successfully tuned regex parameters to identify suspicious PowerShell flag combinations (`-EncodedCommand`, `-ExecutionPolicy Bypass`), verified live alert mapping to MITRE ATT&CK T1059.001, and validated detection responsiveness via simulated endpoint execution.
* **Milestones (July 24):** Configured automated EDR response capabilities within `ossec.conf` on `WAZUH-01`. Tied Level 10+ detection thresholds to dynamic `netsh` host isolation commands, enforcing automated 10-minute remediation windows upon detection of malicious endpoint execution behaviors.
* **Technical Takeaway:** Shifted SIEM posture from passive detection to active automated response (SOAR). Automated host-level containment reduces Mean Time to Respond (MTTR) while maintaining non-destructive, timed rule rollbacks (`timeout=600`) to minimize operational downtime.
